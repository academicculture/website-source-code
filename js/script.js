console.log("%c Github %c", "background:#333333; color:#ffffff", "", "https://github.com/imainjs/hexo-theme-mainjs");
...
(function ($) {
  ...
  var fn = {
    showMenu: function () {
      ...
      fn.hideFab();
      mainjs.utils.bindKeyup(27, function () {
        fn.hideMenu();
    ...
    navbar: {
      mobile: function () {
        ...
        $(window).on("scroll", mainjs.utils.throttle(function () {
  ...
  var action = {
    ...
    carrier: function () {
      $(".j-carrier-btn").on("click", mainjs.utils.throttle(function () {
        $(".j-carrier-data").select();
        document.execCommand("Copy");
        mainjs.zui.message({ text: '已复制到剪切板', type: 'success' });
      }, 1000));
    },
    navbar: function () {
      $(window).resize(mainjs.utils.throttle(function () {
        mainjs.utils.isDesktop() && fn.navbar.desktop();
        mainjs.utils.isMobile() && fn.navbar.mobile();
      }, 1000)).resize();
    ...
    notification: function () {
      ...
      var playList = list.filter(function (item) {
        return item.enable && mainjs.utils.isDuringDate(item.startTime, item.endTime) && item;
      });
      playList.forEach(function (item) {
        mainjs.zui.notification({ title: item.title, content: item.content, delay: delay });
      });
    },
    search: function () {
